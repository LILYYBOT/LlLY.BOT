...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("funcionando")
        await self.highrise.walk_to(Position(15.0 , 0.0 , 7.0 , "FrontRight"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala")   
        await self.highrise.chat(f"Seja bem vindo, aproveita a sala, faça novos amigos e divirta-se.\n\nQualquer problema ou dúvidas Falar com os ADMs")
        await self.highrise.send_whisper(user.id,f"\npara saber a lista de emotes use:!emote list\nlista de comandos use: !help\npara usar loop: !loop jetpack ou !loop 98")
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if message.lower().startswith("!tipall ") and user.username == "YourTrag3dy":
              parts = message.split(" ")
              if len(parts) != 2:
        ...
        if message.lower().startswith("!tipme ") and user.username== "YourTrag3dy":
                try:
        ...
        if        message.startswith("/tele") or              message.startswith("/tp") or              message.startswith("/fly") or     message.startswith("!tele") or      message.startswith("!tp") or     message.startswith("!fly"):
          if user.username =="YourTrag3dy":            await self.teleporter(message)
        ...
        if message.lower().lstrip().startswith(("Ajuda","/ajuda","-help","ajuda","!help","Help","!ajuda","/help","help")):
            await self.highrise.send_whisper(user.id, f"Olá Prescisa de ajuda? 👋\n\nComandos livres:\n!wave @user 1-10\n!thumbs @user 1-10\n!wink @user 1-10\n!clap @user 1-10\n!heart @user 1-10\n/fly 0,0,0\n/tele @user\n/emotes\n/loop emote nome | /loop 1-98\n!follow")
        ...
        if              message.startswith("Jetpack All") or                              message.startswith("/emote all jetpack") or       message.startswith("!emote all jetpack"):
          if user.username in ["ShoKytoo","YourTrag3dy","__tvoy__"]:
        ...
        if message == "!fit 1" and user.username in ["ShoKytoo","YourTrag3dy","__tvoy__"]:
        ...
        if message == "!fit 2" and user.username in ["ShoKytoo","YourTrag3dy","__tvoy__"]:
        ...
        if message == "!fit 3"  and user.username in ["ShoKytoo","YourTrag3dy","__tvoy__"]:
...
Item(type='clothing', amount=1, id='earrings-n_room12019goldhoops', account_bound=False, active_palette=-1),
        ...
        if message == "!fit 4" and user.username in ["ShoKytoo","YourTrag3dy","__tvoy__"]:
        ...
        if message == "!fit 5" and user.username in ["ShoKytoo","YourTrag3dy","__tvoy__"]:
        ...
        if message == "!fit 6" and user.username in ["ShoKytoo","YourTrag3dy","__tvoy__"]:
        ...
        if message == "!fit 7" and user.username in ["ShoKytoo","YourTrag3dy","__tvoy__"]:
        ...
        if message == "!fit 8" and user.username in ["ShoKytoo","YourTrag3dy","__tvoy__"]:
...
Item(type='clothing', amount=1, id='earrings-n_room12019goldhoops', account_bound=False, active_palette=-1),
        ...
        if message == "!fit 9" and user.username in ["ShoKytoo","YourTrag3dy","__tvoy__"]:
        ...
        if message == "!fit 10" and user.username in ["ShoKytoo","YourTrag3dy","__tvoy__"]:
...
Item(type='clothing', amount=1, id='earrings-n_room12019goldhoops', account_bound=False, active_palette=-1),
        ...
        if message == "!fit 11" and user.username in ["ShoKytoo","YourTrag3dy","__tvoy__"]:
...
Item(type='clothing', amount=1, id='bag-n_registrationavatars2023furrytail', account_bound=False, active_palette=-1),
        ...
        if message.lower().lstrip().startswith(("Thumbs","!thumbs","/thumbs","-thumbs","thumbs")) and user.username in ["ShoKytoo","YourTrag3dy","__tvoy__"]:
          try:
              ...
              if 1 <= num_hearts <= 100:
                  ...
                  await self.highrise.send_whisper(user.id,f"\n🚫 {user.username} Numero invalido de reaçoes escolha um numero entre: 1-100")
          except ValueError:
              await self.highrise.send_whisper(user.id,f"🚫\n {user.username} Use: !thumbs @user 1-10")

        if message.lower().lstrip().startswith(("Wave","!wave","/wave","-wave","wave")) and user.username in ["ShoKytoo","YourTrag3dy","__tvoy__"]:
          try:
        ...
        if message.lower().lstrip().startswith(("Clap","!clap","/clap","-clap","👏","clap")) and user.username in ["ShoKytoo","YourTrag3dy","__tvoy__"]:
          try:
        ...
        if message.lower().lstrip().startswith(("Heart","!heart","/heart","-heart","❤️","heart")) and user.username in ["ShoKytoo","YourTrag3dy","__tvoy__"]:
          try:
